...
static void treeNodeDeref(HctTreeNode *pNode){
  assert( pNode->nRef>0 );
  pNode->nRef--;
  if( pNode->nRef==0 ){
    sqlite3_free(pNode);
...
int sqlite3HctTreeInsert(
  ...
  pNew = (HctTreeNode*)hctMallocZero(sizeof(HctTreeNode) + nData);
  if( pNew==0 ){
    return SQLITE_NOMEM;
  }
  pNew->iKey = iKey;
  ...
  if( iStmt<pTree->iStmt ){
    if( iStmt==0 ){
      ...
      for(pNode=pTree->pRollback; pNode!=pStop; pNode=pPrev){
        pPrev = pNode->pPrev;
        if( pNode->pClobber ) treeNodeDeref(pNode->pClobber);
        treeNodeDeref(pNode);
      }
...
int sqlite3HctTreeCsrClose(HctTreeCsr *pCsr){
  if( pCsr ){
    ...
    *pp = pCsr->pCsrNext;
    if( pCsr->pReseek ) treeNodeDeref(pCsr->pReseek);
    sqlite3_free(pCsr);
  }
  return SQLITE_OK;
}
